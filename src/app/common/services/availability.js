...
class AvailabilityService {
  ...
  getCalendarConfig( scope, calendarView ) {
    ...
    const config = {
      ...
      select: ( start, end, jsEvent, view, resource ) => {
        ...
        let availability = this.AvailabilityModel.getCollection().find( event => moment( start ).isSame( event.start, 'day' ) && (calendarView === 'me' ? true : event.employeeId === resourceId) ) || {
  ...
  getCalendarData( scope ) {
    const availabilities = ( start, end, timezone, cb ) => {
      const query = scope.calendarView === 'me' ? {
        employeeId: this.AuthenticationService.getCurrentUser().id,
        start: start.format( AVAILABILITY_DATE_FORMAT ),
        end: end.format( AVAILABILITY_DATE_FORMAT )
      } : {
        start: start.format( AVAILABILITY_DATE_FORMAT ),
        end: end.format( AVAILABILITY_DATE_FORMAT )
      };

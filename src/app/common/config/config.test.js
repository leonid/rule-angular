...
class OnConfigTest {
  ...
  static configFactory( $provide, localStorageServiceProvider ) {
    const localStoragePrefix = 'rules-test'
    ...
    // use "rules-test" as a localStorage name prefix so app doesn’t accidently read data from another app using the same variable names
...
class OnRunTest {
  ...
  static runFactory( $httpBackend, localStorageService ) {
    ...
    // clear all locale storage for rules-test except 'user' and 'token' data

...
class AnimatedButton {
  ...
  template() {
    return '<button type="submit" class="btn {{buttonClass}} {{buttonSize}} {{onlyIcons}} btn-ng-bs-animated clearfix" ng-disabled="{{formIsInvalid}}">' +
  ...
  link( scope, element ) {
    ...
    scope.$watch( 'result', function ( newValue ) {
      ...
      if ( newValue === 'success' ) {
        ...
        this.$timeout( endAnimation, scope.options.animationCompleteTime )
      }
      if ( newValue === 'error' ) {
  ...
  static directiveFactory() {
    AnimatedButton.instance = new AnimatedButton()
    return AnimatedButton.instance
  }
}
